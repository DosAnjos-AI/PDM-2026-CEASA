...
n_obs_demo) para os 6 modelos gold e grava o resultado em
pdm-ceasa.metricas.comparacao_modelos.

Faz: roda sql/consultas/metricas/03_previsoes_demo_gold.sql via "bq query" e
captura a saida em memoria; le, so leitura,
~/Dropbox/PDM/recorte/cotacoes_recorte_demo.csv; normaliza produto com a
mesma regra da Silver (reproduzida aqui, identica a scripts/metricas_demo.py);
casa previsao com observado por (produto, classe, data) -- a previsao de
periodo (semana/mes) e a mesma repetida para as 3 datas de demo, por
construcao da consulta (convencao declarada no cabecalho do arquivo .sql);
calcula mae/mse/rmse/n por (modelo, granularidade); grava os 4 valores via
UPDATE em pdm-ceasa.metricas.comparacao_modelos, com WHERE camada='gold' AND
modelo=... AND granularidade=... (executado via "bq query"); imprime um
resumo por modelo/granularidade (casados/nao casados, metricas).

Nao faz: nao contem CREATE MODEL nem CREATE OR REPLACE MODEL -- nao retreina
nada. Nao grava o recorte em disco nem em nenhuma tabela do BigQuery -- o
CSV e lido e mantido so em memoria, e a unica escrita no BigQuery e o UPDATE
das 4 colunas de demo para as 6 linhas gold. Nao calcula R2 no bloco de demo
(mesma decisao do script irmao para bronze/silver -- poucas observacoes
tornariam o R2 enganoso). Nao mexe nas 6 linhas de bronze/silver.
"""
...
SQL_PREVISOES = RAIZ_REPO / "sql" / "consultas" / "metricas" / "03_previsoes_demo_gold.sql"
...
MODELOS = [
    ("regressao_linear", "semanal"),
    ("regressao_linear", "mensal"),
    ("regressao_arvore", "semanal"),
    ("regressao_arvore", "mensal"),
    ("serie_arima", "semanal"),
    ("serie_arima", "mensal"),
]
...
def roda_previsoes_demo() -> list[dict]:
    """Executa sql/consultas/metricas/03_previsoes_demo_gold.sql via bq CLI
    e devolve as linhas como lista de dicts. So leitura de modelo ja
...
def calcula_metricas(previsoes: list[dict], observado: dict) -> dict:
    ...
    mae_demo/mse_demo/rmse_demo/n_obs_demo por (modelo, granularidade). Nao
    calcula R2 -- fora de escopo do bloco de demo."""
    por_modelo: dict[tuple[str, str], dict] = {
        (modelo, granularidade): {"erros_abs": [], "erros_sq": [], "casaram": 0, "nao_casaram": 0}
        for modelo, granularidade in MODELOS
    }

    for linha in previsoes:
        modelo = linha["modelo"]
        granularidade = linha["granularidade"]
        chave_modelo = (modelo, granularidade)
...
def grava_metricas(metricas: dict) -> None:
    """Gera um UPDATE por (modelo, granularidade) e executa tudo numa unica
    chamada ao bq CLI (script multi-statement). Grava so os 4 valores
    agregados -- nenhum dado do recorte entra na instrucao alem dos numeros
    finais. WHERE inclui camada='gold' para nao atingir as 6 linhas de
    bronze/silver, que ja tem granularidade='diario'."""
    comandos = []
    for (modelo, granularidade), valores in metricas.items():
        ...
        comandos.append(
            ...
            f"WHERE camada = 'gold' AND modelo = '{modelo}' AND granularidade = '{granularidade}';"
        )
    script_sql = "\n".join(comandos)
    subprocess.run(
...
def main() -> None:
    ...
    print("\n[FATO] resumo por modelo/granularidade:")
    for modelo, granularidade in MODELOS:
        valores = metricas[(modelo, granularidade)]
        print(
            f"  gold/{modelo}/{granularidade}: casaram={valores['casaram']} "
    ...
    print(f"\n[FATO] UPDATE executado em {TABELA_METRICAS} para os 6 modelos gold")
